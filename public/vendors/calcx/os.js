...
if ($counter > 0) {
    ...
    $('#table_servicos tr').each(function () {
        ...
        /*Armazena na variável servico, o valor contido no input 'produto_descricao'*/
        ...
        select: function (event, ui) {
            ...
            if (ui.item.value === 'Serviço não encontrado') {
                ...
                var markup = '<tr>\
                    ...
                    <td><input title="Descrição do servico" type="text" name="servico_descricao[]" value="' + servico_descricao + '" class="servico_descricao form-control form-control-sm form-control-user input-sm bg-light" data-cell="B' + i + '" readonly></td>\
                    <td><input title="Valor unitário do servico" name="servico_preco[]" value="' + servico_preco + '" class="form-control form-control-user form-control-sm input-sm text-right money pr-1 bg-light" data-cell="C' + i + '" data-format="R$ 0,0.00" readonly></td>\
                    <td><input title="Digite a quantidade apenas em número inteiros" type="text" inputmode="numeric" pattern="[-+]?[0-9]*[.,]?[0-9]+" name="servico_quantidade[]" value="" class="qty form-control form-control-sm form-control-user text-center" data-cell="D' + i + '" data-format="0[.]00" required></td>\
                    <td><input title="Insira o desconto" name="servico_desconto[]" class="form-control form-control-sm form-control-user input-sm text-right" value="0" data-cell="E' + i + '" data-format="0,0[.]00 %" required></td>\
                    <td><input title="Valor total do servico selecionado" name="servico_item_total[]" class="form-control form-control-sm form-control-user input-sm text-right pr-1 bg-light" data-cell="F' + i + '" data-format="R$ 0,0.00" data-formula="D' + i + '*(C' + i + '-(C' + i + '*E' + i + '))" readonly></td>\
                    <td class="text-center"><input type="hidden" name="valor_desconto_servico[]" data-cell="H' + i + '"  data-format="R$ 0,0.00" data-formula="((C' + i + '*D' + i + ')-F' + i + ')"><button title="Remover o servico" class="btn-remove btn btn-sm btn-primary"><i class="fa fa-trash"></i></button></td>\
                </tr>';
